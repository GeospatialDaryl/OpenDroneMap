...
class ODMJob:
    ...
    def resize(self):
        ...
        for objPhotos in self.listObjPhotos:
            if objPhotos.isOk:
                ...
                thisCmd = BIN_PATH + os.sep + dictOSBins['exeJhead'] + " " + \
                    objPhotos.dictStepVals["step_0_resizedImage"] 
                
                file_resolution = run_and_grep( thisCmd , "Resolution")
    ...
    def getKeypoints(self):
        ...
        def workerConvert(inJPG1):
            '''  Worker function for Keypoints
            '''               
            subprocess.call(["convert","-format","pgm",inJPG1],shell=False)    
        
        def workerSift(inJPG1,inJPG2):
            '''
            '''
            subprocess.call([BIN_PATH+dictOSBins['exeSift'], ""])
